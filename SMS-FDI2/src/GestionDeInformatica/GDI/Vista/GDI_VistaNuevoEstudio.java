...
public class GDI_VistaNuevoEstudio extends JFrame {
	...
	public GDI_VistaNuevoEstudio(boolean modificacion, TransferPlantilla estudio, int fila,ArrayList<TransferPlantilla> arrayPlantillas) {
		...
		if(modificacion == true) {
			...
			variables=FactoriaControlador.getInstancia().leerPlantilla(estudio.getID(),estudio.getNOM(),estudio.getETA(), variables);
		...
		btnSave.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(modificacion == true)FactoriaControlador.getInstancia().eliminarEstudio(arrayPlantillas, fila);
				FactoriaControlador.getInstancia().anadirEstudio(plantilla);
				FactoriaControlador.getInstancia().anadirPlantilla(id,nombre,etapa,parametros);
				dispose();
				FactoriaControlador.getInstancia().pantalla6(id);
			}
		});
