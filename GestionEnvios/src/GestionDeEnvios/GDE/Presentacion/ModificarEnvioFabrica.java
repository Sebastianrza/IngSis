...
public class ModificarEnvioFabrica extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					ModificarEnvioFabrica frame = new ModificarEnvioFabrica();
	 ...
	 */
	public ModificarEnvioFabrica() throws FileNotFoundException {
		this.identificador = new DefaultComboBoxModel<String>();
		identificador.removeAllElements();
		EnvioFa = dao.addPedidoFabrica(nuevoPedido);
		for ( int i = 0; i<EnvioFa.size(); i++) {
			
			identificador.addElement(EnvioFa.get(i).getID_Envio());
		}
		setTitle("Modificar Envio Fabrica");
